  ...
  <div class="page-wrapper fade-in">
    ...
    <main class="container d-flex justify-content-center align-items-center">
      <div class="gasto-form-card">
        <form @submit.prevent="editarIngreso">
          ...
          <div class="form-group">
            ...
            <select v-model.number="categoria" id="categoria" class="form-select">
...
const obtenerIngreso = async () => {
  ...
  const res = await fetch(`${API_URL}/api/ingresos/consultone`, {
    method: 'POST',
    headers: {
...
const editarIngreso = async () => {
  if(id) {
    const token = sessionStorage.getItem('token')
    const res = await fetch(`${API_URL}/api/ingresos/update`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    const token = sessionStorage.getItem('token')
    const res = await fetch(`${API_URL}/api/ingresos/create`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    const data = await res.json()
    if (res.ok) {
      Swal.fire('Ingreso creado', '', 'success')
      router.push('/dashboard')
    } else {
